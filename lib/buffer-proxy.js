...
class BufferProxy {
  ...
  constructor ({id, uri, text, history, operations, decNet, hostPeerId, siteId, didDispose}) {
    this.id = id;
    ...
    this.subscriptions.add(
      this.decNet.onNotification(`/buffers/${id}`, this.receiveUpdate.bind(this))
    );
    if (this.isHost) {
      this.subscriptions.add(
        this.decNet.onRequest(`/buffers/${id}`, this.receiveFetch.bind(this)),
        this.decNet.onNotification(`/buffers/${id}/save`, this.receiveSave.bind(this))
      )
    } else {
      this.subscriptions.add(
        this.decNet.onNotification(`/buffers/${id}/disposal`, this.dispose.bind(this))
      )
    }
  ...
  dispose () {
    ...
    if (this.isHost) this.decNet.notify({channelId: `/buffers/${this.id}/disposal`});
    this.emitDidDispose()
  }
  ...
  requestSave () {
    ...
    this.decNet.notify({recipientId: this.hostPeerId, channelId: `/buffers/${this.id}/save`})
  }
  ...
  broadcastUpdate (buffer) {
    this.decNet.notify({channelId: `/buffers/${this.id}`, body: buffer})
  }
};
