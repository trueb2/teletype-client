...
module.exports =
class Portal {
  constructor ({id, networkPeerId, hostPeerId, siteId, connectionTimeout, tetherDisconnectWindow}) {
    this.id = id;
    this.networkPeerId = networkPeerId;
    ...
    if (this.isHost) {
      this.bindPeerIdToSiteId(this.networkPeerId, this.siteId);
      this.nextSiteId = 2;
      this.nextBufferId = 1;
      this.nextEditorId = 1;
      this.disposables.add(
        // TODO: DecNet event for receive Subscription
    ...
    this.disposables.add(
      // TODO: DecNet event for receiving updates
      // this.router.onNotification(`/portals/${id}`, this.receiveUpdate.bind(this)),
      // TODO: DecNet event for member leaving
  ...
  async initialize () {
    try {
      // TODO: Start DecNet if not started
  ...
  async join () {
    ...
    // TODO: DecNet Request to join portal by remote host id
    // const rawResponse = await this.router.request({recipientId: this.hostPeerId, channelId: `/portals/${this.id}`});
    const rawResponse = null;
    const portalSubscriptionResponse = Messages.PortalSubscriptionResponse.toObject(Messages.PortalSubscriptionResponse.decode(rawResponse));

    portalSubscriptionResponse.siteIdsByPeerId.forEach((siteId, peerId) => {
      this.bindPeerIdToSiteId(peerId, siteId);
    });
    // TODO: Resolve peerId for network maybe with DecNet userID
    this.siteId = this.siteIdsByPeerId.get(this.userId);
    // this.siteId = this.siteIdsByPeerId.get(this.network.getPeerId());

    portalSubscriptionResponse.editorProxiesMetadata.forEach((editorProxyMetadata) => {
      this.deserializeEditorProxyMetadata(editorProxyMetadata);
    });

    portalSubscriptionResponse.tethers.forEach((tether) => {
      this.tethersByFollowerId.set(tether.followerSiteId, {
        leaderId: tether.leaderId,
        state: tether.state
      });
    });

    portalSubscriptionResponse.activeBufferProxies.forEach((activeBufferProxy) => {
      this.deserializeBufferProxy(activeBufferProxy);
    });

    portalSubscriptionResponse.activeEditorProxies.forEach((activeEditorProxy) => {
      this.deserializeEditorProxy(activeEditorProxy);
    });

    portalSubscriptionResponse.activeEditorProxiesBySiteId.forEach((editorProxyId, siteId) => {
  ...
  createBufferProxy (props) {
    ...
    const bufferProxy = new BufferProxy(Object.assign({
      ...
      siteId: this.siteId,
      // TODO: DecNet needs to register for events for notifications and updates
      // router: this.router,
  ...
  async fetchBufferProxy (id) {
    // TODO: DecNet request buffer from remote host peer
    // const response = await this.router.request({recipientId: this.hostPeerId, channelId: `/buffers/${id}`});
    const response = null;
    if (response.ok) {
      const bufferProxy = Messages.BufferProxy.toObject(Messages.BufferProxy.decode(response.body));
      return this.deserializeBufferProxy(bufferProxy);
    }
  }

  deserializeBufferProxy (message) {
    const bufferProxy = BufferProxy.deserialize(message, {
      // TODO: Need to change ProtoBuf to remove router or something because BufferProxy needs it
      // router: this.router,
      hostPeerId: this.hostPeerId,
      siteId: this.siteId,
  ...
  createEditorProxy (props) {
    ...
    const editorProxy = new EditorProxy(Object.assign({
      id,
      siteId: this.siteId,
      // TODO: DecNet this also needs to register for events for notifications and requests
      // router: this.router,
  ...
  async fetchEditorProxy (id) {
    ...
    // TODO: DecNet request editor from remote host peer
    // const response = await this.router.request({recipientId: this.hostPeerId, channelId: `/editors/${id}`});
    const response = null;
    if (!response.ok) return;

    const editorProxy = Messages.EditorProxy.toObject(Messages.EditorProxy.decode(response.body));
    const bufferProxy = await this.findOrFetchBufferProxy(editorProxy.bufferProxyId);

    if (bufferProxy) {
      const editorProxy = this.deserializeEditorProxy(editorProxy);
  ...
  deserializeEditorProxy (message) {
    const editorProxy = EditorProxy.deserialize(message, {
      // TODO: EditorProxy wants router but can't have it!
      // router: this.router,
      siteId: this.siteId,
  ...
  deserializeEditorProxyMetadata (message) {
    const editorProxyMetadata = EditorProxyMetadata.deserialize(message, {
      siteId: this.siteId,
      // TODO: Everyone wants the router thing to register for notifications and requests
      // router: this.router,
      didDispose: () => {
  ...
  getActiveSiteIds () {
    // TODO: Get active site users from DecNet with the correct peerIDs
  ...
  assignNewSiteId (peerId) {
    ...
    const siteAssignment = new Messages.PortalUpdate.SiteAssignment({
      peerId,
      siteId
    });
    const portalUpdate = new Messages.PortalUpdate({
      siteAssignment
    });
    const buffer = Messages.PortalUpdate.encode(portalUpdate).finish();

    // TODO: DecNet notify new portal
    // this.router.notify({channelId: `/portals/${this.id}`, body: updateMessage.serializeBinary()})
  }

  sendSubscriptionResponse (requestId) {
    const portalSubscriptionResponse = new Messages.PortalSubscriptionResponse({
      siteIdsByPeerId: new Map(),
      activeEditorProxyIdsBySiteId: new Map(),
      editorProxiesMetadata: new Array()
    });
    this.siteIdsByPeerId.forEach((siteId, peerId) => {
      portalSubscriptionResponse.siteIdsByPeerId.set(peerId, siteId);
    });
    this.editorProxiesById.forEach((editorProxy) => {
      portalSubscriptionResponse.editorProxiesMetadata.push(editorProxy.getMetadata());
    });
    ...
    this.activeEditorProxiesBySiteId.forEach((editorProxy, siteId) => {
      if (editorProxy) {
        ...
        portalSubscriptionResponse.activeEditorProxiesBySiteId.set(siteId, editorProxy.id)
      }
    });
    portalSubscriptionResponse.activeBufferProxies = Array.from(activeBufferProxiesById.values());
    portalSubscriptionResponse.activeEditorProxies = Array.from(activeEditorProxiesById.values());

    const tethers = [];
    this.tethersByFollowerId.forEach((tether, followerId) => {
      const tetherMessage = new Messages.Tether({
        followerSiteId: followerId,
        leaderSiteId: tether.leaderId,
        state: tether.state
      });
      tethers.push(tetherMessage);
    });
    portalSubscriptionResponse.tethers = tethers;

    // TODO: DecNet respond to subscription request
    const buffer = Messages.PortalSubscriptionResponse.encode(portalSubscriptionResponse).finish();
    // this.router.respond({requestId, body: response.serializeBinary()})
  }

  async receiveUpdate ({senderId, body}) {
    const portalUpdate = Messages.PortalUpdate.toObject(Messages.PortalUpdate.decode(body));

    if (portalUpdate.editorProxyCreation) {
      this.receiveEditorProxyCreation(portalUpdate.editorProxyCreation);
    } else if (portalUpdate.editorProxySwitch) {
      const senderSiteId = this.siteIdsByPeerId.get(senderId);
      await this.receiveEditorProxySwitch(senderSiteId, portalUpdate.editorProxySwitch);
    } else if (portalUpdate.siteAssignment) {
      this.receiveSiteAssignment(portalUpdate.siteAssignment);
    } else if (portalUpdate.tetherUpdate) {
      this.receiveTetherUpdate(portalUpdate.tetherUpdate);
    } else {
  ...
  receiveEditorProxyCreation (editorProxyCreationMessage) {
    this.deserializeEditorProxyMetadata(editorProxyCreationMessage.editorProxyMetadata);
    this.delegate.didChangeEditorProxies()
  }

  async receiveEditorProxySwitch (senderSiteId, editorProxySwitch) {
    const editorProxyId = editorProxySwitch.editorProxyId;
  ...
  receiveSiteAssignment (siteAssignment) {
    const siteId = siteAssignment.siteId;
    const peerId = siteAssignment.peerId;
    this.bindPeerIdToSiteId(peerId, siteId);
    // TODO: DecNet get peerID (maybe userID)
  ...
  receiveTetherUpdate (tetherUpdate) {
    ...
    const followerSiteId = tetherUpdate.followerSiteId;
    const leaderSiteId = tetherUpdate.leaderSiteId;
    const tetherState = tetherUpdate.state;
  ...
  setFollowState (newState, newLeaderId) {
    ...
    if (oldState !== newState || oldLeaderId !== newLeaderId) {
      const tetherMessage = new Messages.Tether({
        followerSiteId: this.siteId,
        leaderSiteId: newLeaderId,
        state: newState
      });
      const portalUpdate = new Messages.PortalUpdate({
        tetherUpdate: tetherMessage
      });
      const buffer = Messages.PortalUpdate.encode(portalUpdate).finish();

      // TODO: DecNet notify portals of leader change
      // this.router.notify({channelId: `/portals/${this.id}`, body: updateMessage.serializeBinary()})
    }
  }
  ...
  broadcastEditorProxyCreation (editorProxy) {
    const editorProxyCreation = new Messages.PortalUpdate.EditorProxyCreation({
      editorProxyMetadata: editorProxy.getMetadata().serialize()
    });
    const portalUpdate = new Messages.PortalUpdate({
      editorProxyCreation
    });
    ...
    // TODO: notify portals of new editor proxy
    // this.router.notify({channelId: `/portals/${this.id}`, body: updateMessage.serializeBinary()})
  }

  broadcastEditorProxySwitch (editorProxy) {

    const editorProxySwitch = new Messages.PortalUpdate.EditorProxySwitch({
      editorProxyId: editorProxy.id
    });
    const portalUpdate = new Messages.PortalUpdate({
      editorProxySwitch
    });
    ...
    // TODO: notify portals of switch of editor proxy
    // this.router.notify({channelId: `/portals/${this.id}`, body: updateMessage.serializeBinary()})
  }
