...
module.exports =
class Portal {
  constructor ({id, networkPeerId, hostPeerId, siteId, decNet, connectionTimeout, tetherDisconnectWindow}) {
    this.id = id;
    ...
    if (this.isHost) {
      ...
      this.disposables.add(
        this.decNet.onRequest(`/portals/${id}`, this.receiveSubscription.bind(this))
      )
    } else {
    ...
    this.disposables.add(
      this.decNet.onNotification(`/portals/${id}`, this.receiveUpdate.bind(this)),
  ...
  async join () {
    ...
    const response = await this.decNet.request({recipientId: this.hostPeerId, channelId: `/portals/${this.id}`});
    const portalSubscriptionResponse = Messages.PortalSubscriptionResponse.toObject(Messages.PortalSubscriptionResponse.decode(response.body));
  ...
  async fetchBufferProxy (id) {
    const response = await this.decNet.request({recipientId: this.hostPeerId, channelId: `/buffers/${id}`});
    if (response.ok) {
  ...
  async fetchEditorProxy (id) {
    ...
    const response = await this.decNet.request({recipientId: this.hostPeerId, channelId: `/editors/${id}`});
  ...
  assignNewSiteId (peerId) {
    ...
    this.decNet.notify({channelId: `/portals/${this.id}`, body: buffer})
  }
